...
layers = [[Dense, (512), {'activation': 'relu'}],
           ...
           [Dense, (512), {'activation': 'relu'}]]
m_ini = mm.make(layers,{'shape':(backward*data.shape[-1],)})
...
# test model
#'''
train_gen = series_generator(data[140000:160000], batch_size, d_mean=d_mean, d_std=d_std,
                             backward=backward, forward=forward, mode="train", aug=None)
test_gen = series_generator(data[160000:170000], batch_size, d_mean=d_mean, d_std=d_std,
...
def train(model):
    ...
    hist = model.fit_generator(train_gen, steps_per_epoch=data.shape[0] // batch_size,
...
while time.time()-now<60*60*7:
    input = Input(shape=(backward, data.shape[-1]))
    l_1 = Flatten()(input)
    models.append(mm.make(layers, {'shape': (backward * data.shape[-1],)}))
    l_2 = []
    for mods in models:
        l_2.append(mods(l_1))
