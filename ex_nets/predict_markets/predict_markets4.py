...
from keras.layers import Dense, Input, Concatenate, Flatten, Reshape, Activation
...
def make_deepnet(layers,shape,nodes,depth):
    ...
    for i in range(depth):
        t_layer = mm.make(layers,{'shape':(shape+nodes*i,)})
        _mods.append(t_layer)
    ...
    for i in range(depth):
        ...
        layer = _mods[i](conc)
        agg.append(layer)
        conc=Concatenate()(agg)
    return Model(inputs=input,outputs=layer)

deep1 = make_deepnet(layers,backward*data.shape[-1],nodes,5)
...
def train(model):
    model.compile(optimizer=optimizers.adam(lr=1e-3, beta_2=.85, epsilon=1e-4), loss='mean_squared_error')
...
while time.time()-now<60*60*7:
    input_r = Input(shape=(backward, data.shape[-1]))
    l_1 = Flatten()(input_r)
    for mo in models:
        m_layers = mo.layers
        for l in range(len(m_layers)):
            m_layers[l].trainable=False

    deep2 = make_deepnet(layers, backward*data.shape[-1], nodes, 5)
    ...
    out = Reshape((forward, data.shape[-1]), input_shape=(forward * data.shape[-1],))(l_3)
    model = Model(inputs=input_r, outputs=out)
    #model.summary()
