...
layers = [[LSTM, (nodes), {'activation': 'relu','return_sequences':True,'stateful':True}],
          [LSTM, (nodes), {'activation': 'relu','return_sequences':True,'stateful':True}],
          [LSTM, (nodes), {'activation': 'relu','return_sequences':True,'stateful':True}],
          [LSTM, (nodes), {'activation': 'relu','return_sequences':True,'stateful':True}]]

deep1 = mm.make_stateLSTMdeepnet(layers,(batch_size,1,data.shape[-1]),128,5)


#print('dense1 made')
def makeme():
    input_l = Input(batch_shape=(batch_size,1,data.shape[-1]))
    l_2 = deep1(input_l)
    l_3 = LSTM(1*data.shape[-1],activation='linear',stateful=True,return_sequences=False)(l_2)
    out = Reshape((1,data.shape[-1]), input_shape=(1,1,data.shape[-1],))(l_3)
    #out = Dense(data.shape[-1],activation='linear')(out)
...
def series_generator(data,true, bs, d_mean=1, d_std=1, t_mean=1,t_std=1, backward=12, forward=12, mode="train", aug=None):
    ...
    while True:
        ...
        while len(dataset) < bs:
            ...
            truth.append(np.array(tru).reshape(1,5))
...
# test model
#'''
train_gen = series_generator(data[155000:160000],truth[155000:160000], batch_size, d_mean=d_mean, d_std=d_std,t_mean=t_mean, t_std=t_std,
                             backward=backward, forward=forward, mode="pred", aug=None)
test_gen = series_generator(data[160000:165000],truth[160000:165000], batch_size, d_mean=d_mean, d_std=d_std,t_mean=t_mean, t_std=t_std,
                             backward=backward, forward=forward, mode="pred", aug=None)
...
for i in range(2,3):
    ...
    lr = 1*10**-i
    print ('lr = ',lr)
    optimizer=optimizers.sgd(learning_rate=lr)#, momentum=0.2, nesterov=True)
    model.compile(optimizer=optimizer, loss='mean_squared_error')
    hist.append(model.fit(np.array((data[50000:160000]-d_mean)/d_std).reshape(110000,1,5),
              np.array((truth[50000:160000]-t_mean)/t_std).reshape(110000,1,5),batch_size=batch_size,epochs=1))
'''
...
for i in range(4,5):
    ...
    hist.append(train(model,optimizer=optimizers.sgd(learning_rate=lr, momentum=0.9, nesterov=True), epochs=epoch,
                      callbacks=[CosHotRestart(nb_epochs=epoch,nb_cycles=epoch//10,Pgain=1.1,LRgain=.8,verbose=2,valweight=False,save_model='callbacks/CosHot_weights/')]
                      ))
