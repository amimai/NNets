...
from PyTorch.data_analysis.model1 import lane
...
class GRUNet(nn.Module):
    def __init__(self, lanes, inputs, backwards, output_dim, hidden_dim, batch_size, n_layers, drop_prob=0.40, GPUs=1):
        ...
        for i in range(self.lanes):
            self.lane_list.append(lane(inputs,hidden_dim,self.attn_layers,backwards,drop_prob))
        ...
        for i in range(self.lanes):
            self.dens.append( Deepnet(backwards * hidden_dim * lanes,
                                   [[hidden_dim, hidden_dim],
                                    [hidden_dim, hidden_dim],
                                    [hidden_dim, hidden_dim],
                                    [hidden_dim, hidden_dim]],
        ...
        for i in range(self.lanes):
            self.lin_outs.append(nn.Linear(hidden_dim*2, output_dim))
    ...
    def forward(self, x,y,z, h):
        bs=x.shape[0]
        #print(x.shape)
        out = torch.cat([self.lane_list[0](x).reshape(bs, self.backwards * self.hidden_dim),
                         self.lane_list[1](y).reshape(bs, self.backwards * self.hidden_dim),
                         self.lane_list[2](z).reshape(bs, self.backwards * self.hidden_dim)],1)
        #print(out.shape)
        #out = self.r1_dnet(out)
        tmp = self.r1_dnet(out)

        return self.lin_outs[0](torch.cat([tmp,self.dens[0](out)],1)),\
               self.lin_outs[1](torch.cat([tmp,self.dens[1](out)],1)),\
               self.lin_outs[2](torch.cat([tmp,self.dens[2](out)],1)), h
...
class trainer:
    ...
    def train(self,batch_size,EPOCHS=1,test_loader=None,transform = False):
        ...
        losses = [ls.L3_STD_MSE()] #nn.MSELoss(),nn.L1Loss(),ls.scaled_error_d()#ls.scaled_error_d()
