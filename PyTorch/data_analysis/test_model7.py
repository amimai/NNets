...
class GRUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, backwards, drop_prob=0.2, GPUs=1):
        ...
        for i in range(self.n_layers):
            ...
            self.c_out.append(nn.Linear(hidden_dim*2,hidden_dim))
    ...
    def forward(self, x):
        ...
        for i in range(self.n_layers):
            ...
            # do convolutions
            if i == 0:
                c1 = self.drops[drops](self.relus[rels](self.conv1[i](x.view(-1,1,self.backwards,self.input_dim))))
                c2 = self.drops[drops+1](self.relus[rels+1](self.conv2[i](x.view(-1,1,self.backwards,self.input_dim))))
                cat = self.relus[rels + 2](self.c_out[i](
                    self.relus[rels + 3](torch.cat(
                        (self.c1_lin1[i](c1.view(-1,z1_dim*56*(self.backwards-1*(i+1)))),
                         self.c2_lin1[i](c2.view(-1,z1_dim*168*(self.backwards-2*(i+1))))), 1
                    ))
                ))
            else:
                ...
                cat = torch.cat(
                    (self.relus[rels + 2](self.c_out[i](self.relus[rels + 3](torch.cat((self.c1_lin1[i](c1.view(-1,z1_dim*56*(self.backwards-1*(i+1)))),self.c2_lin1[i](c2.view(-1,z1_dim*168*(self.backwards-2*(i+1))))), 1))
                )),cat),1)
