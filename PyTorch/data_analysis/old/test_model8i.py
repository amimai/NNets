...
class GRUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, batch_size, n_layers, backwards, drop_prob=0.20, GPUs=1):
    ...
    def forward(self, x, h):
        ...
        out = self.relu( self.fc1(out.reshape(bs, self.backwards * self.hidden_dim)))
        out = self.drop(out)
...
class trainer:
    def __init__(self, t_x,t_y,batch_size=None,model=None, history=None, optimizer = None, scheduler=None, lr=1e-3):
        ...
        self.coshot_cycle =3* 2477#7433#825#275#
        self.t_x = t_x
        self.t_y = t_y
            ...
            for x, label in train_loader:
                ...
                if roll_loss<self.loss:
                    ...
                    if self.age*self.age_factor>self.losscount:
                        self.loss *=1.001**self.age_factor # increase the loss threshold if trapped in blind alley
                        self.age = 0
                        self.age_factor *= 1.5
