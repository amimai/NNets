...
backwards = 128
spacing = 24 #spacing used to determine the space between each datapoint grouping data between
...
good_cols = wr.get_cols(dat, ['bidopen','bidhigh','bidlow'])#,'tick'])#['EUR/USD'])  # ['bidopen'])# , 'tick'
dat = dat[good_cols]
...
#target_cols = wr.get_cols(dat, ['bidopen'])#['EUR/USD'])  # ['bidopen'])# , 'tick'
#tru = tru[target_cols]
...
class trainer:
    def __init__(self, t_x,t_y,batch_size=None,model=None, history=None, optimizer = None, scheduler=None, lr=1e-3):
        ...
        self.coshot_cycle =3* 2477#7133#825#275#
        self.t_x = t_x
        self.t_y = t_y
    ...
    def train(self,batch_size,EPOCHS=1,test_loader=None):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for x, label in train_loader:
                ...
                t,f=torch.ones(*out.shape)*.5,torch.ones(*out.shape)*-.5
                # binarise label loss to force decision
                loss = criterion(out, torch.where(label>0,t,f).to(device).float())
                # add regular loss
                loss = torch.add(criterion(out, label.to(device).float()),loss)
                ...
                if roll_loss<self.loss:
                    ...
                    if self.age*self.age_factor>self.losscount:
                        self.loss *=1.0001**self.age_factor # increase the loss threshold if trapped in blind alley
                        self.age = 0
                        self.age_factor *= 1.5
...
if __name__ == '__main__':
    ...
    pl_diff.mean(axis=1).plot()
    (pl_diff.mean(axis=1).rolling(200).sum() / 200).plot()

    from matplotlib import pyplot as plt
    (db_df.iloc[:, 18:27].mean(axis=1).rolling(200).sum() / 200).plot()
    (db_df.iloc[:, 3:9].mean(axis=1).rolling(200).sum() / 200).plot()
    df_t.iloc[:, 3:9].plot()
    df_p.iloc[:, 3:9].plot()
    df_t.iloc[:, 18:27].plot()
    df_p.iloc[:, 18:27].plot()
