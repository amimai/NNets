...
batch_size=3**3 #300, 17s, .79loss #3000, 3s, .81loss
learn_rate=1e-4
EPOCHS=50
...
backwards = 128
spacing = 6 #spacing used to determine the space between each datapoint grouping data between
...
class trainer:
    def __init__(self, t_x,t_y,batch_size=None,model=None, history=None, optimizer = None, scheduler=None, lr=1e-3):
        ...
        self.coshot_cycle =3* 7433#2477#275#825#
        self.t_x = t_x
        self.t_y = t_y
    ...
    def transform_loader(self):
        ...
        self.trainloader = None
        n_x = self.t_x + (((np.random.rand(*self.t_x.shape) - .5) / 10))
        n_y = self.t_y + (((np.random.rand(*self.t_y.shape) - .5) / 10))
