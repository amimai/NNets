...
from PyTorch.data_analysis.model1 import lane2,lane_merge,self_attn
...
class GRUNet(nn.Module):
    def __init__(self, lanes, inputs, backwards, output_dim, hidden_dim, batch_size, n_layers, drop_prob=0.40, GPUs=1):
        ...
        for i in range(self.lanes):
            ...
            self.attn0.append(self_attn(hidden_dim,backwards,drop_prob))
        self.attn0.append(self_attn(hidden_dim, backwards, drop_prob))
        self.attn0.append(self_attn(hidden_dim, backwards, drop_prob))
    ...
    def forward(self, x,y,z, h):
        ...
        tmp = self.r1_dnet(self.attn0[-1](out).reshape(bs, self.backwards * self.hidden_dim))
        out0 = self.attn0[0](out).reshape(bs, self.backwards * self.hidden_dim)
        out1 = self.attn0[1](out).reshape(bs, self.backwards * self.hidden_dim)
        out2 = self.attn0[2](out).reshape(bs, self.backwards * self.hidden_dim)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for x,y,z,t0,t1,t2 in train_loader:
                ...
                for i in range(len(losses)):
                    ...
                    tmp_loss = losses[i](o2, t2.to(device).float())
    ...
    def evaluate(self, test_loader, batch_size, pred=False):
        ...
        with torch.no_grad():
            ...
            for x,y,z,t0,t1,t2 in test_loader:
                ...
                loss = criterion(o2, t2.to(device).float())
                losses += loss.item()
            loss = losses / i
            if pred:
