...
class GRUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, backwards, drop_prob=0.2, GPUs=1):
        ...
        self.conv1a = torch.nn.Conv2d(1,20,(5,1),stride=(1,1),padding=(2,0))     # expand in c axis
        self.conv1b = torch.nn.Conv2d(20, 40, (5, 3), stride=(1, 1), padding=(2, 1))  # expand in c axis
        self.conv1c = torch.nn.Conv2d(40, 80, (5, 3), stride=(1, 1), padding=(2, 1))  # expand in c axis
        self.pool1 = torch.nn.MaxPool2d((7,1),stride=(2,1),padding=(3,0))       # maxpool to get features
        self.redu1 = torch.nn.Conv2d(80,10,(1,1),stride=(1,1),padding=(0,0))     # reduce c
        self.conv_r1 = int(10 * self.backwards/2 * self.input_dim)

        self.conv2a = torch.nn.Conv2d(10,40,(5,3),stride=(1,1),padding=(2,1))     # reduce in ohl axis
        self.conv2b = torch.nn.Conv2d(40, 80, (5, 3), stride=(1, 1), padding=(2, 1))  # reduce in ohl axis
        self.conv2c = torch.nn.Conv2d(80, 120, (5, 3), stride=(1, 3), padding=(2, 0))  # reduce in ohl axis
        self.pool2 = torch.nn.MaxPool2d((7,1),stride=(2,1),padding=(3,0))       # extract features out of c2
        self.redu2 = torch.nn.Conv2d(120, 20, (1, 1), stride=(1, 1), padding=(0, 0))  # reduce c
        self.conv_r2 = int(20 * self.backwards/4 * self.input_dim/3)

        self.conv3 = torch.nn.Conv2d(20, 80, (7, 3), stride=(1, 1), padding=(3, 1))  # reduce in ohl axis
        self.pool3 = torch.nn.MaxPool2d((3, 3), stride=(2, 1), padding=(1, 1))  # extract features out of c2
        self.redu3a = torch.nn.Conv2d(80, 30, (1, 1), stride=(1, 1), padding=(0, 0))  # reduce c
        self.redu3b = torch.nn.Conv2d(30, 10, (1, 1), stride=(1, 1), padding=(0, 0))  # reduce c
    ...
    def forward(self, x):
        ...
        out = F.leaky_relu(self.conv1a(out))
        out = F.leaky_relu(self.conv1b(out))
        out = self.conv1c(out)
        ...
        out = F.leaky_relu(self.conv2a(out))
        out = F.leaky_relu(self.conv2b(out))
        out = self.conv2c(out)
        ...
        out = F.leaky_relu(self.redu3a(out))
        out = F.leaky_relu(self.redu3b(out))

        r3 = self.r3_dnet(self.drop(out).view(bs, self.conv_r3))

        out = self.drop(torch.cat((r1,r2, r3),1))
        #out = self.drop(torch.cat((out, ), 1))
