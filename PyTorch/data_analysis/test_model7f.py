...
good_cols = wr.get_cols(dat, ['EUR'])#['EUR/USD'])  # ['bidopen'])# , 'tick'
dat = dat[good_cols]
...
target_cols = wr.get_cols(dat, ['EUR/USD'])#['EUR/USD'])  # ['bidopen'])# , 'tick'
tru = tru[target_cols]
...
class GRUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, batch_size, n_layers, backwards, drop_prob=0.5, GPUs=1):
        ...
        self.GRU = GRU(input_size=input_dim, hidden_size=hidden_dim, num_layers=n_layers)
        '''
...
class trainer:
    def __init__(self, t_x,t_y,batch_size=None,model=None, history=None, optimizer = None, scheduler=None, lr=1e-3):
        ...
        self.lr = lr
        self.epoch = 0
        self.scaling = .65 #how much LR decreases every micro cycle
        self.aneal_rate = 3**3  #how often we update LR to best LR
        self.coshot_cycle = 2477#275#825#7433#
        self.t_x = t_x
        self.t_y = t_y
    ...
    def transform_loader(self):
        ...
        n_x = None
        n_y = None
        self.trainloader = DataLoader(n_train_data, shuffle=False, batch_size=self.batch_size, drop_last=True, num_workers=16)
    ...
    def train(self,batch_size,EPOCHS=1,test_loader=None):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            #h = self.model.module.init_hidden(device)
            h = None
