...
class GRUNet(nn.Module):
    def __init__(self, lanes, inputs, backwards, output_dim, hidden_dim, batch_size, n_layers, drop_prob=0.40, GPUs=1):
        ...
        self.feed_list0 = nn.ModuleList()
        self.feed_list1 = nn.ModuleList()
        self.feed_list2 = nn.ModuleList()
        for i in range(self.lanes):
            self.lane_list.append(lane2(inputs,hidden_dim,self.attn_layers,backwards,drop_prob))
            self.feed_list0.append(lane_merge(hidden_dim,backwards,drop_prob))
            self.feed_list1.append(lane_merge(hidden_dim, backwards, drop_prob))
            self.feed_list2.append(lane_merge(hidden_dim, backwards, drop_prob))


        self.gru1 = GRU(int(input_dim/1), hidden_dim, n_layers, drop_prob, batch_first=True)

        self.r1_dnet = Deepnet(backwards * hidden_dim * lanes,
    ...
    def forward(self, x,y,z, h):
        bs=x.shape[0]
        #print(x.shape)
        t2 = self.lane_list[2](z)
        t1 = self.lane_list[1](y)
        t0 = self.lane_list[0](x)

        out0 = self.feed_list0[2](t1, t2)
        out0 = self.feed_list0[1](t0, out0)
        out0 = out0.reshape(bs, self.backwards * self.hidden_dim)

        out1 = self.feed_list0[2](t1, t2)
        out1 = self.feed_list0[1](t0, out1)
        out1 = out1.reshape(bs, self.backwards * self.hidden_dim)

        out2 = self.feed_list0[2](t1, t2)
        out2 = self.feed_list0[1](t0, out2)
        out2 = out2.reshape(bs, self.backwards * self.hidden_dim)

        tmp = self.r1_dnet(torch.cat([out0,out1,out2],1))

        return self.lin_outs[0](torch.cat([tmp,self.dens[0](out0),out0],1)),\
               self.lin_outs[1](torch.cat([tmp,self.dens[1](out1),out1],1)),\
               self.lin_outs[2](torch.cat([tmp,self.dens[2](out2),out2],1)), h
...
class trainer:
    ...
    def train(self,batch_size,EPOCHS=1,test_loader=None,transform = False):
        ...
        losses = [ls.L3_MSE_2()] #,nn.L1Loss(),ls.scaled_error_d()ls.L3_MSE_2()#ls.scaled_error_d()##ls.L3_STD_MSE()#nn.MSELoss()
