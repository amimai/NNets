...
class trainer:
    ...
    def train(self,batch_size,EPOCHS=1,test_loader=None,transform = False):
        ...
        losses = [ls.scaled_error()] #nn.MSELoss(),nn.L1Loss()
...
if __name__ == '__main__':
    ...
    for i in range(5):
        thresh = .05+i/10
        if tmp is None:
            tmp = ((df_p.iloc[:, ::3] > thresh) * df_t.iloc[:, ::3] - (df_p.iloc[:, ::3] < -thresh) * df_t.iloc[:,::3]).cumsum().mean()
        else:
            tmp.append(((df_p.iloc[:, ::3] > thresh) * df_t.iloc[:, ::3] - (df_p.iloc[:, ::3] < -thresh) * df_t.iloc[:,::3]).cumsum().mean())

    del pred,targ,loss,dtb,dpb,db_df,df_p,df_t,df_d,negative,positive,pl_diff,df_lwe
    ...
    from PyTorch.data_analysis.datastore import train_data, test_data
    from PyTorch.data_analysis.test_model9a import *
    savedir = 'PyTorch/data_analysis/saved_mod/test_9/a'
    train = trainer()
    train.boot(batch_size, hidden_dim=hidden_dim, output_dim=output_dim, n_layers=n_layers, backwards=backwards,
               GPUs=[0], trainloader=train_loader)
    train.load(savedir,20)
    for i in range(5):
        train.train(batch_size, EPOCHS=3, test_loader=test_loader)
        train.save(savedir)
    train.graph()
